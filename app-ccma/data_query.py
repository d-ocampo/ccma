...
def buscar_datos_existe(buscar, base, data, clientes_col):
    # buscar='1-1007845123'
    ...
    if base == "Experiencia":
        existe['características']['EXPERIENCIA'] = data[data['CEDULA_NEW']
                                                        == ident].to_dict('records')
    elif base == "Llamadas":
        existe['características']['LLAMADAS'] = data[(data['TIPO DE DOCUMENTO'] == tipo_id) & (
            data['CEDULA_NEW'] == ident)].to_dict('records')
    elif base == "Cuentas":
        existe['características']['CUENTAS'] = data[(data['Tipo_documento_value'] == tipo_id) & (
            data['CEDULA_NEW'] == ident)].to_dict('records')
    elif base == "Demanda":
        existe['características']['DEMANDA'] = data[data['CEDULA_NEW']
                                                    == ident].to_dict('records')
    elif base == "Interés":
        existe['características']['INTERES'] = data[data['CEDULA_NEW']
                                                    == ident].to_dict('records')
    elif base == "Contactos":
        existe['características']['CONTACTOS'] = data[(data['TIPO DE DOCUMENTO'] == tipo_id) & (
            data['CEDULA_NEW'] == ident)].to_dict('records')
...
# nuevo='1-1015438726'
def agregar_nuevo(nuevo, base, data, clientes_col):
    ...
    # Segun la selección de la base llenar ese espacio
    if base == "Experiencia":
        dict_personas['características']['EXPERIENCIA'] = data[data['CEDULA_NEW']
                                                               == ident].to_dict('records')
    elif base == "Llamadas":
        dict_personas['características']['LLAMADAS'] = data[(
            data['TIPO DE DOCUMENTO'] == tipo_id) & (data['CEDULA_NEW'] == ident)].to_dict('records')
    elif base == "Cuentas":
        dict_personas['características']['CUENTAS'] = data[(
            data['Tipo_documento_value'] == tipo_id) & (data['CEDULA_NEW'] == ident)].to_dict('records')
    elif base == "Demanda":
        dict_personas['características']['DEMANDA'] = data[data['CEDULA_NEW']
                                                           == ident].to_dict('records')
    elif base == "Interés":
        dict_personas['características']['INTERES'] = data[data['CEDULA_NEW']
                                                           == ident].to_dict('records')
    elif base == "Contactos":
        dict_personas['características']['CONTACTOS'] = data[(
            data['TIPO DE DOCUMENTO'] == tipo_id) & (data['CEDULA_NEW'] == ident)].to_dict('records')
