...
dem_base_melt['ASISTENCIA']=dem_base_melt['ID'].apply(lambda x: 0 if pd.isna(x) else 1)
...
data_e_e = Dataset.load_from_df(demanda_base[['userID', 'itemID', 'rating']], reader)
...
def recomendacion_cf(data,algo,event,n):
    ...
    predictions=algo.test(test)
    #base para listado
    df_rec = pd.DataFrame(predictions, columns=['uid', 'iid', 'rui', 'est', 'details'])
    #filtrar por evento
    if len(df_rec[df_rec['iid']==event])<n:
        df_rec=df_rec.sample(n)
    else:
        df_rec=df_rec[df_rec['iid']==event].sample(n)
    most_freq=Counter(" ".join(df_rec["iid"].astype(str)).split()).most_common(20)
    df_rec=df_rec.fillna(0)
    df_rec['SCORE']=abs(df_rec['est']-df_rec['rui'])
    return df_rec[['uid','SCORE']],most_freq
...
def recomendacion_coseno(data,event,n,idf,tipo):       
    ...
    if tipo=='evento':
        data['data_text']=data.apply(lambda x: str(x['ID']+','+x['TEMAS_INTERES']).replace(",", " ") ,axis=1)
    elif tipo=='llamada':
        data['data_text']=data.apply(lambda x: str(x['ID']+','+x['TEMA DEL SERVICIO1']).replace(",", " "),axis=1)
    ...
    if len(data[data['ID'].str.contains(event)]['CEDULA_NEW'])==0:
        ...
        cedula_index=data[data['ID'].str.contains(event)]['CEDULA_NEW'].sample(1).to_list()
    ...
    most_freq=Counter(" ".join(data_rec["ID"].replace(","," ")).split()).most_common(20)
...
#Crear matrix tfidf
creacion_matrix_idf(int_base_g.apply(lambda x: str(x['ID']+','+x['TEMAS_INTERES']).replace(",", " ")),'tfdidf interes - evento')
#cargar idf
...
creacion_matrix_idf(llam_base.apply(lambda x: str(x['ID']+','+x['TEMA DEL SERVICIO1']).replace(",", " "),axis=1),'tfdidf llamada - evento')
